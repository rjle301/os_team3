...
static void clk_isr(int vector, int code) {
  ...
  if ((system_time % SEC_TO_TICKS(SYSTEM_STATUS)) == 0) {
    ...
    sprint(tbuf, " curr %d, Qs: R[%d] S[%d] Z[%d] B[%d] I[%d] W[%u] ?[%u]",
           current ? current->pid : -1, 1, que_length(sleeping),
           que_length(zombie), que_length(blocked), que_length(sioread),
           nst[STATE_WAITING], nunk);
...
void clk_init(void) {
  ...
  outb(PIT_CONTROL_PORT, PIT_0_SELECT | PIT_LOAD | PIT_SQUARE | PIT_DECIMAL);
