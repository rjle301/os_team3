...
OFFCFLAGS :=
#OFFCFLAGS := -DCREATE_HEADER_FILE

Offsets:        util/Offsets.c
	@mkdir -p $(BUILD)
	$(CC) -mx32 -std=c99 $(OFFCFLAGS) $(INCLUDES) -fno-builtin \
		-o Offsets util/Offsets.c

#
# This rule prevents us from updating the include/offsets.h file if the
# new version is identical to the old version. This prevents us from
# triggering a recompile of other things when that isn't necessary.
#
include/offsets.h:	Offsets
	@./Offsets -h
	-@sh -c 'cmp -s include/offsets.h $(BUILD)/new_offsets.h || \
		(cp $(BUILD)/new_offsets.h include/offsets.h; \
		echo "\n*** NOTE - updated include/offsets.h, rebuild!" ; \
		rm -f $(BUILD)/new_offsets.h)'
