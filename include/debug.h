...
#define IDT_PRINT(v)                                                           \
  cio_printf(" [%02x] = %08x %08x @ %08x " #v "\n", (v),                       \
             *(uint32_t *)(IDT_ADDR + ((v) << 3)),                             \
             *(uint32_t *)(IDT_ADDR + ((v) << 3) + 4),                         \
             (IDT_ADDR + ((v) << 3)))

// longword printing
#define MEM_PRINT_LW(base, v)                                                  \
  cio_printf(" [%02x] = %08x @ %08x " #v "\n", (v), (base)[(v)],               \
             &((base)[(v)]));
...
#define TRACE                                                                  \
  (TRPCB | TRQUEUE | TRSCHED | TRDISP | TRSCALLS | TRSRETS | TREXIT | TRKMEM | \
   TRINIT | TRSIO_ST | TRSIO_RD | TRSIO_WR | TRSTK | TRSTKS | TRKMIN |         \
   TRKMFL | TRRBT)
...
#define TRACING_RBT ((TRACE & TRRBT) != 0)
